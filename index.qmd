...
`col_foldername`   | (str) Case-sensitive column name for the column containing the deployment or folder name.
`col_timebottom`   | (str) Case-sensitive column name for the column containing the "time-on-bottom" timestamps. Expected format of the timestamps is `HH:MM:SS:FF`.
...
`fps`                       | (int, float, or str) Desired frame rate (frames per second) of the output video. Set this if a different frame is desired than that of the input videos. Set to "auto" to match the frame rate of the input videos (default).
...
: Processing settings that can be adjusted if or as desired.  These can be omitted from the configuration file if the default values are acceptable.
