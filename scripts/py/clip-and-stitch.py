...
Key Features:
    ...
    * Asynchronous Throttled Upload: Streams completed videos to a GCP bucket
        in background threads using `rclone` with network bandwidth throttling.
    * Independent Subsystem Timing: Separately records and reports encoding
        runtime, network upload runtime, and total overall wall-clock runtime.
...
Required Dependencies:
    ...
    * FFmpeg/ffprobe: Must be installed and accessible via system path 
        or config file.
    * rclone: Command line tool for cloud transfers with bandwidth throttling.
...
def clean_and_validate_config(config: dict):
    ...
    a leading ".", strips 'gs://' prefixes from GCP bucket paths, and ensures
    the path ends with a "/" to ensure it is treated as a file prefix.
    ...
    # Clean gs:// prefix and ensure trailing slash on GCP bucket path
    if config.get('gcp_upload', False) and 'gcp_bucket_path' in config:
        raw_bucket = str(config['gcp_bucket_path']).strip()
        if raw_bucket.lower().startswith('gs://'):
            raw_bucket = raw_bucket[5:]
        elif raw_bucket.lower().startswith('gs:\\'):
            raw_bucket = raw_bucket[5:]
        config['gcp_bucket_path'] = raw_bucket.rstrip('/') + '/'
...
def build_bwlimit_schedule(peak_limit: str | None) -> str | None:
    """Generates an rclone schedule string enforcing peak_limit during business
    hours (Mon-Fri 07:00-17:00) and 'off' during nights and weekends.
    
    Arguments
    ---------
    peak_limit (str | None): Bandwidth limit during workday hours (e.g., '5M')
    
    Returns
    -------
    str | None: The formatted rclone schedule string, or None if disabled.
    """
    # Handle different possibilties for peak_limit
    if not peak_limit or str(peak_limit).strip().lower() in ('off', 'none', 'false', '0'):
        return None
    
    # Create the string key to be passed to rclone bandwidth throttle flag
    peak = str(peak_limit).strip()
    days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri']
    parts = []
    for day in days:
        parts.append(f"{day}-00:00,off")
        parts.append(f"{day}-07:00,{peak}")
        parts.append(f"{day}-17:00,off")
    parts.append("Sat-00:00,off")
    parts.append("Sun-00:00,off")
    return " ".join(parts)

def get_rclone_executable() -> str | None:
    """Locates the rclone executable on system PATH or standard install paths."""
    # Look for rclone
    rclone_exec = shutil.which("rclone")
    if rclone_exec:
        return rclone_exec
    
    # Possible local locations to find rclone
    possible_paths = [
        os.path.join(os.getenv("LOCALAPPDATA", ""), "Programs", "rclone", "rclone.exe"),
        os.path.join("C:\\", "Program Files", "rclone", "rclone.exe")
    ]
...
def check_rclone_auth(config: dict) -> bool:
    """Verifies rclone availability and GCS bucket access before starting.
    
    Arguments
    ---------
    config (dict): configuration settings dictionary
    """
    # Confirm rclone is installed and executable
    rclone_exec = get_rclone_executable()
    if not rclone_exec:
        print("\n❌ ERROR: 'rclone' executable not found. Is rclone installed and in your PATH?")
        return False

    # Assemble cloud destination
    remote_name = "gcp_remote:"
    bucket_path = config['gcp_bucket_path']
    full_target = f"{remote_name}{bucket_path}"

    # Test rclone
    try:
        cmd = [rclone_exec, "lsf", full_target, "--gcs-bucket-policy-only", "--max-depth", "1"]
        result = subprocess.run(cmd, capture_output=True, text=True)
        
        if result.returncode != 0:
            print(f"\n[!] RCLONE BUCKET ACCESS ERROR: Cannot access target '{full_target}'")
            print(f"Details: {result.stderr.strip()}")
            return False
            
        return True
    except Exception as e:
        print(f"\nERROR: An unexpected error occurred while verifying rclone: {e}")
        return False

def upload_single_file_rclone(output_path: str, config: dict, rclone_exec: str) -> bool:
    """Uploads a single video file to GCP using rclone with bandwidth throttling."""
    filename = os.path.basename(output_path)
    remote_name = "gcp_remote:"
    target_remote_file = f"{remote_name}{config['gcp_bucket_path']}{filename}"
    ...
    # Handle reprocess mode vs standard upload protection
    if config.get('reprocess', False):
        # Force rclone to overwrite cloud file unconditionally when reprocessing
        cmd.append("--ignore-times")
    else:
        # Protect existing files and compare checksums when not reprocessing
        cmd.extend(["--checksum", "--ignore-existing"])

    bwlimit_str = build_bwlimit_schedule(config.get('rclone_peak_bwlimit'))
    if bwlimit_str:
        cmd.extend(["--bwlimit", bwlimit_str])

    try:
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode == 0:
            if config.get('delete_local_after_upload', False) and os.path.exists(output_path):
                os.remove(output_path)
            return True
        else:
            tqdm.write(f"  [!] Rclone Upload Error for {filename}: {result.stderr.strip()}")
            return False
    except Exception as e:
        tqdm.write(f"  [!] Rclone Upload Exception for {filename}: {str(e)}")
        return False
...
def process_deployments(config_path: str = 'configurations.yml', process=True):
    ...
    # Confirm rclone bucket access if gcp_upload is True
    rclone_exec = None
    if config['gcp_upload']:
        ...
        if 'gcp_bucket_path' not in config:
            ...
            return False
        if not check_rclone_auth(config=config):
            return False
    ...
    if config['gcp_upload'] and config['delete_local_after_upload']:
        print("  > Mapping remote bucket directory using rclone to cache existing deployments...", flush=True)
        remote_name = "gcp_remote:"
        full_remote_path = f"{remote_name}{config['gcp_bucket_path']}"
        
        ls_remote = subprocess.run(
            [rclone_exec, "lsf", full_remote_path, "--gcs-bucket-policy-only", "--max-depth", "1"],
    ...
    # Total progress bar setup for video encoding
    bar_format = "{desc}: {percentage:3.0f}%|{bar}| {n_fmt}/{total_fmt}"
    pbar = tqdm(total=len(tasks), position=0, desc="Encoding Video", bar_format=bar_format)

    # Initialize execution timers and ThreadPoolExecutor for background uploads
    encoding_start = time.perf_counter()
    upload_start_time = None
    upload_futures = []
    upload_executor = ThreadPoolExecutor(max_workers=1) if config['gcp_upload'] and process else None

    # Instantiate ProcessPoolExecutor explicitly to avoid context-manager
    # blocking on KeyboardInterrupt
    executor = ProcessPoolExecutor(max_workers=config['num_workers'], initializer=init_worker)
    ...
    try:
        ...
        while futures_list:
            ...
            for future in done:
                ...
                if result['status'] == "ERROR":
                    ...
                    # Submit completed video to background upload queue immediately
                    if upload_executor:
                        if upload_start_time is None:
                            upload_start_time = time.perf_counter()
                            
                        tqdm.write(f"  [->] Handing off {folder_id} to rclone uploader...")
                        up_future = upload_executor.submit(
                            upload_single_file_rclone, 
                            result['output_path'], config, rclone_exec
                        )
                        upload_futures.append((folder_id, result['output_path'], up_future))
        ...
        # Capture pure Video Encoding Runtime
        encoding_duration = time.perf_counter() - encoding_start

        # Wait for remaining background uploads to finish after encoding completes
        upload_duration = 0.0
        if upload_futures:
            print("\n  > All encodings complete. Finishing remaining background cloud uploads...\n", flush=True)
            upload_pbar = tqdm(total=len(upload_futures), position=0, desc="Cloud Uploading", bar_format=bar_format)
            for folder_id, path, up_fut in upload_futures:
                up_success = up_fut.result()
                if up_success:
                    tqdm.write(f"  > {folder_id} uploaded to GCP successfully via rclone.")
                else:
            ...
            if upload_start_time is not None:
                upload_duration = time.perf_counter() - upload_start_time
            
        if upload_executor:
            upload_executor.shutdown(wait=True)

    except KeyboardInterrupt:
        print("\n\n[!] PROCESS CANCELED: Interrupted by user (Ctrl+C). Halting workers immediately...", flush=True)
        pbar.close()
        
        # Forcefully terminate all running worker processes in the ProcessPoolExecutor
        for process_item in getattr(executor, '_processes', {}).values():
            try:
    ...
    summary_lines.append(f"Number of cloud upload failures:     {len(failed_uploads)}")
    ...
    if failed_uploads:
        summary_lines.append(f"\n{'!'*15} CLOUD UPLOAD FAILURES {'!'*15}")
        summary_lines.append(f"  -> {len(failed_uploads)} video(s) failed to stream to Google Cloud Storage:")
    ...
    # Helper function to format duration strings nicely
    def format_duration(seconds: float) -> str:
        """Formats raw seconds into detailed hours, minutes, and seconds."""
        if seconds > 3600:
            hrs = int(seconds // 3600)
            mins = int((seconds % 3600) // 60)
            secs = int(seconds % 60)
            return f"{seconds / 3600:.2f} hours ({hrs}h {mins}m {secs}s)"
        elif seconds > 60:
            mins = int(seconds // 60)
            secs = int(seconds % 60)
            return f"{seconds / 60:.2f} minutes ({mins}m {secs}s)"
        return f"{seconds:.2f} seconds"

    # Log and print individual and combined timing metrics
    total_duration = time.perf_counter() - process_start
    
    timing_msg = (
        f"  Execution Timing Breakdown:\n"
        f"    -> Total Encoding Runtime (CPU/GPU): {format_duration(encoding_duration)}\n"
    )
    if config['gcp_upload']:
        timing_msg += f"    -> Total Upload Runtime (Network):   {format_duration(upload_duration)} (Overlapped)\n"
    timing_msg += f"    -> Total Overall Wall-Clock Runtime: {format_duration(total_duration)}\n"
    
    log_and_print(timing_msg, config['log_file'])
